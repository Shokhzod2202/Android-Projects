...
class DrawingView(context: Context,attrs:AttributeSet): View(context,attrs) {
    ...
    private val mDeletedPaths = ArrayList<CustomPath>()
    ...
    fun undo(){
        if(mPaths.isNotEmpty()) {
            mDeletedPaths.add(mPaths[mPaths.size - 1])
            mPaths.removeAt(mPaths.size - 1)
            invalidate()
        }
    }
    fun redo(){
        if(mDeletedPaths.isNotEmpty()) {
            mPaths.add(mDeletedPaths[mDeletedPaths.size - 1])
            mDeletedPaths.removeAt(mDeletedPaths.size - 1)
            invalidate()
        }
